    ...
    public class ImportListSyncService : IExecute<ImportListSyncCommand>
    {
        ...
        private List<Album> Sync()
        {
            ...
            foreach (var report in reports)
            {
                ...
                {
                    var mappedAlbum = _albumSearchService.SearchForNewAlbum(report.Album, report.Artist, report.ReleaseDate)
                        .FirstOrDefault();
                ...
                {
                    artistsToAdd.Find(s => s.ForeignArtistId == report.ArtistMusicBrainzId).Albums.Add(new Album { ForeignAlbumId = report.AlbumMusicBrainzId });
                }
            }
